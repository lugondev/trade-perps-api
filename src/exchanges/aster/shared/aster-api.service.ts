...
export class AsterApiService {
   ...
   */
  async hmacPost<T = any>(
    endpoint: string,
    params: Record<string, any>,
  ): Promise<AsterApiResponse<T>> {
    try {
      ...
      const signature = crypto
        .createHmac('sha256', this.credentials.apiSecret as string)
        .update(qs)
        .digest('hex');
  ...
  private async generateAsterSignature(config: AxiosRequestConfig): Promise<AsterSignatureParams> {
    try {
      if (!this.credentials.user || !this.credentials.signer || !this.credentials.privateKey) {
        throw new Error(
          'Aster credentials (user, signer, privateKey) are required for signature generation',
        );
      }
  ...
  debugCredentials(): any {
    return {
      ...
      apiKeyPrefix: this.credentials.apiKey
        ? this.credentials.apiKey.substring(0, 8) + '...'
        : 'NOT_SET',
    };
  }
}
